...
const Checkout = () => {
  const reduxStateCart = useSelector((global) => global?.CartReducer?.cart);
  const reduxStateUser = useSelector(
    (global) => global?.UserReducer?.user?.user
  );
  ...
  const payNow = () => {
    let options = {
      key: "rzp_live_ERmr2kJdmEXNlT",
      key_secret: "uoSZ54ZTN28JEuKWFBX8679M",
      amount:
  ...
  return (
    <div className="my-3 flex flex-col gap-3 items-center">
      ...
      <div className="w-full md:w-3/5 rounded-lg  py-3 shadow-lg bg-white flex flex-col items-center">
        ...
        <button
          onClick={payNow}
          className="flex items-center gap-2 justify-center my-4 md:my-8 w-full px-4 md:w-4/5 md:px-0 h-14 text-white font-medium text-lg bg-zomato_red rounded-lg"
        >
